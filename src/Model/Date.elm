...
monthsBetween dA dB =
     ...
     case (month(dA) , month(dB) ) of
         (Just a,Just b)  -> if compare dA dB == GT then Just ( (   (year(dA) - year(dB)) * 12) + ( monthToInt  a ) - ( monthToInt  b ) ) else Just ( ((year(dB) - year(dA)) * 12) + (monthToInt  b ) - ( monthToInt a ) )
...
compare (Date d1) (Date d2) =
      case ( d1.month , d2.month )  of
        (Just a, Just b ) -> if (d1.year > d2.year) then GT else if (d1.year < d2.year) then LT else if (  ( monthToInt a ) > (monthToInt b) ) then GT else if ((monthToInt a) < (monthToInt b)) then LT else EQ
        (Just a, Nothing) -> if (d1.year > d2.year) then GT else if (d1.year < d2.year) then LT else LT
        (Nothing,Just a)  -> if (d1.year > d2.year) then GT else if (d1.year < d2.year) then LT else GT
        (Nothing, Nothing) -> if (d1.year > d2.year) then GT else if (d1.year < d2.year) then LT else EQ
...
view (Date d) =
    let
      isThereMonth a =
         let
            stringMonth=a.month |> Maybe.map monthToString
         in
         case stringMonth  of
             ( Just b )-> String.fromInt a.year ++b
             _->String.fromInt a.year
    in
    div [] [
    h3 [][text  ( isThereMonth d)]
    ]
